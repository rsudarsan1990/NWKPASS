				...
				LIMIT 1000;
					...
					LOOP
						...
						IF (v_comment IS NULL ) THEN
							 if (LENGTH(l_COMMENT_DESC(budg_indx)) > 1996) then
								v_comment := SUBSTR(l_COMMENT_DESC(budg_indx),0,1996)||'...';
							else	
								v_comment := l_COMMENT_DESC(budg_indx);	
							end if;
							DBMS_OUTPUT.PUT_LINE ('IF: '||v_comment);
						ELSE
							v_comment := fstringConcat(l_COMMENT_DESC(budg_indx),v_comment);
											...
											LOOP 
												...
												LIMIT 1000;
													...
													if (v_comment is null) then
														...
														v_comment := fstringConcat( v_comment , l_COMMENT_DESC(budg_indx) ); 
													end if;
													END LOOP;		
											EXIT WHEN Chart_E_Budget_Comments%NOTFOUND;
											END LOOP;
                     ...
                     where nc_pass_trans_b_id = trans_b_rec.PASS_TRANS_ID and  POSN_PROPOSED_ACCT_PERCENT = 0;
                      ...
                      IF( (v_FISC_YEAR > v_extract_year  ) OR 
                              ...
                              if trans_b_rec.PASS_COAS_CODE = 'E' then
									...
									if (LENGTH(v_NBRPTOT_COMMENT) > 1996) then
										v_comment := SUBSTR(v_NBRPTOT_COMMENT,0,1996)||'...';
									else	
										v_comment := v_NBRPTOT_COMMENT;	
									end if;	
                              else
                    ...
                    DBMS_OUTPUT.PUT_LINE (chr(10)||'v_date : ' || v_eff_date||chr(10));
                      ...
                      and PWRTCPC_ORGN_CODE_TS like trans_b_rec.POSN_ORGN_CODE;
                          ...
                          IF  ( (trans_b_rec.POSN_COAS_CODE = 'H') and (trans_type = 'RECLASSIFICATION') and (v_EXEMPT_IND = 'Y' ) ) then 
                                ...
                                --char E epaf codes
                                /*
                                EPEXP ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã…â€œ Exempt ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã…â€œ El Paso - PASS
                                EPNEP ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã…â€œ Non-Exempt ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã…â€œ El Paso - PASS
